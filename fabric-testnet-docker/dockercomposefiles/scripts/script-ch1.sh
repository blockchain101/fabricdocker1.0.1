...
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/bctrustmachine.cn/orderers/orderer.bctrustmachine.cn/msp/cacerts/ca.bctrustmachine.cn-cert.pem
...
createChannel() {
	setGlobals 0
		...
		peer channel create -o $CORE_ORDERER_ADDRESS -c $CHANNEL_NAME -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel/channel-$ORGS_PROFILE-$CHANNEL_NAME.tx >&log.txt
	else
		peer channel create -o $CORE_ORDERER_ADDRESS -c $CHANNEL_NAME -f /opt/gopath/src/github.com/hyperledger/fabric/peer/channel/channel-$ORGS_PROFILE-$CHANNEL_NAME.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
	fi
	res=$?
	cat log.txt
...
updateAnchorPeer() {
        ...
        if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer channel create -o $CORE_ORDERER_ADDRESS -c $CHANNEL_NAME -f ./channel/channel-$ORGS_PROFILE-$CHANNEL_NAME-anchor${CORE_PEER_LOCALMSPID}.tx >&log.txt
	else
		peer channel create -o $CORE_ORDERER_ADDRESS -c $CHANNEL_NAME -f ./channel/channel-$ORGS_PROFILE-$CHANNEL_NAME-anchor${CORE_PEER_LOCALMSPID}.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
	fi
	res=$?
	cat log.txt
